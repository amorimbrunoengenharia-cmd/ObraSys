...
content = content.replace(
  'RefreshCw, History, Sparkles',
  'RefreshCw, History, Sparkles, Bot, Send'
);

// 2. State
const stateAnchor = "const [revisions, setRevisions] = useState<any[]>([]);";
const stateVars = `const [revisions, setRevisions] = useState<any[]>([]);
...
// 4 & 5. UI (Chat Modal & FAB)
const uiAnchor = "      {/* Revisions Modal */}";
...
content = content.replace(uiAnchor, chatUI);

fs.writeFileSync(file, content);
console.log('Injected Option A (Chat UI)');
