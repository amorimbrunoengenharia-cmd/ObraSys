...
content = content.replace(
  ...
  "jobRoleId: '', companyId: '', projectIds: [], baseSalary: '', status: 'Ativo', regime: 'CLT', encargos: '68',"
);
...
content = content.replace(
  ...
  "baseSalary: emp.baseSalary?.toString() || '',\n                            status: emp.status || 'Ativo',\n                            regime: emp.regime || 'CLT',\n                            encargos: emp.encargos?.toString() || '68',"
);

content = content.replace(
  "jobRoleId: '', companyId: '', projectIds: [], baseSalary: '', status: 'Ativo',",
  "jobRoleId: '', companyId: '', projectIds: [], baseSalary: '', status: 'Ativo', regime: 'CLT', encargos: '68',"
); // For the reset in Admitir
...
// 6. Update UI for Base Salary to use formatter and add Regime/Encargos
const oldSalaryHTML = `
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase">Salário Base (R$)</label>
                      <input type="number" min="0" step="0.01" value={newEmpForm.baseSalary} onChange={e => setNewEmpForm({...newEmpForm, baseSalary: e.target.value})} className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-rose-500" />
                    </div>`;

const newSalaryAndRegimeHTML = `
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase">Salário Base (R$)</label>
...
// Let's use regex to safely replace the old salary HTML if formatting varies slightly
const salaryRegex = /<div>\s*<label className="text-xs font-bold text-slate-500 uppercase">Salário Base \(R\$\)<\/label>\s*<input type="number" min="0" step="0.01" value=\{newEmpForm.baseSalary\} onChange=\{e => setNewEmpForm\(\{\.\.\.newEmpForm, baseSalary: e.target.value\}\)\} className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-rose-500" \/>\s*<\/div>/;

if (salaryRegex.test(content)) {
  content = content.replace(salaryRegex, newSalaryAndRegimeHTML);
  fs.writeFileSync(file, content, 'utf8');
  console.log("updateSalaryRegime.js applied successfully!");
} else {
  console.log("Could not find the Salary HTML string!");
  // fallback for input without min and step
  const fallbackRegex = /<div>\s*<label className="text-xs font-bold text-slate-500 uppercase">Salário Base \(R\$\)<\/label>\s*<input value=\{newEmpForm.baseSalary\} onChange=\{e => setNewEmpForm\(\{\.\.\.newEmpForm, baseSalary: e.target.value\}\)\} className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-rose-500" \/>\s*<\/div>/;
  if(fallbackRegex.test(content)) {
      content = content.replace(fallbackRegex, newSalaryAndRegimeHTML);
      fs.writeFileSync(file, content, 'utf8');
      console.log("updateSalaryRegime.js applied successfully with fallback!");
  } else {
      console.log("Fallback failed as well. We need to manually inspect.");
  }
}
