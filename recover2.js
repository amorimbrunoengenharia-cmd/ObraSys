...
const anchor = `                      <h4 className="font-bold text-slate-700 dark:text-slate-200 group-hover:text-blue-500">{task.name}</h4>
                    </button>
                  ))}`;
...
if (idx !== -1) {
    const endIdx = content.indexOf('<Link href="/" className="p-2 hover:bg-slate-100', idx);
