...
// Fix button for Magic Search
const buttonRegex = /<button onClick=\{handleSearchRef\} className="px-6 bg-blue-600 text-white font-black text-sm rounded-2xl shadow-lg active:scale-95 transition-all">\s*BUSCAR\s*<\/button>/;
...
// Loading State in UI
const loadingRegex = /\{isSearchingRef \? \(/;
if (loadingRegex.test(content)) {
    content = content.replace(loadingRegex, "{(isSearchingRef || isAiSearching) ? (");
    count++;
}

// FAB and Chat Modal for Option A
const uiRegex = /\{\/\* Revisions Modal \*\/\}/;
...
if (uiRegex.test(content)) {
    content = content.replace(uiRegex, chatUI);
    count++;
}

fs.writeFileSync(file, content);
console.log('UI injected successfully. Modifications: ' + count);
